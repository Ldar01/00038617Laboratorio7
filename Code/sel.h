...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau,kappa,delta,lambda,ny,Ae,J,D;
    
    //Componentes de K
    // [ A+K  -L+C ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixL,matrixD, matrixC;
    ...
    kappa = m.getParameter(CONST_KAPPA);
    ...
    //Preparando matrixG Ahora conocida como matrixL (En clase conocida simplemente como G)
    delta = m.getParameter(CONST_DELTHA);
    ...
    //calculateBPrima(BPrima);
    //productRealMatrix(J/(6*delta*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixL);

    //Preparando matrixC
    lambda = m.getParameter(CONST_LAMBDA);
    productRealMatrix((lambda*J*2)/(18*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixC);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixL,matrixC,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float psi_x = m.getParameter(CONST_PSI_X);
    float psi_y = m.getParameter(CONST_PSI_Y);
